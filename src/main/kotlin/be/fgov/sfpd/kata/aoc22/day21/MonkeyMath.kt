...
fun List<MathMonkey>.findYellOf(targetMonkey: String): Long {
    ...
    while(targetMonkey !in valueMonkeys.keys){
        val yellingMonkeys = operationMonkeys.filter { monkey ->
            monkey.nameMonkey1 in valueMonkeys.keys && monkey.nameMonkey2 in valueMonkeys.keys
        }
        yellingMonkeys.forEach {
            valueMonkeys[it.name] = it.operator.operation(valueMonkeys[it.nameMonkey1]!!,valueMonkeys[it.nameMonkey2]!!)
...
enum class MonkeyOperator(val sign: String, val operation: (Long, Long) -> Long) {
    PLUS("+", { a, b -> a + b }),
    MINUS("-", { a, b -> a - b }),
    MULTIPLY("*", { a, b -> a * b }),
    DIVIDE("/", { a, b -> a / b });
